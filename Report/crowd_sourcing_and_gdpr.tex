...
\begin{document}
	...
	\startsection
		Generally, today's crowd-sourcing methods are GDPR-conform and can still be carried out as before. (Comment PG: Why? We need to have good argumentation for this statement!) \\
		...
		First of all, it must be ensured that the "Right to Restriction" and the "Right to Object" are respected, as there must be an option to prevent the data processing of a certain individual. For example due to the high-quality cameras that are available for tracking purposes, such as traffic flow surveillance, public spaces which are not part of the actual video scanning can be intruded and people which are staying in these spaces can be recognized due to the progress in facial recognition algorithms. (Comment PG: Limitation: we don't know if there is another law that allows this) \\
	...
	\startsection
		...
		\begin{enumerate}[(a)]
			...
			\item If the crowd-sourcing method is not used to achieve a goal of public interest the data must be deleted on request and its processing must be stopped.
		\end{enumerate}
		...
		For services and application where realtime is relevant, as the energy and water consumption in smart homes, for example in the Schoonschipp project in Amsterdam (subsection \ref{Schoonschipp}), whose smart grid system relies on that data, the General Data Protection Regulation can be a hard to implement directive, because the inhabitants are "transparent human beings" as the data from the homes is publicly known. Therefore the consent part of GDPR is significant such that the inhabitant are well informed and authorize the service provider to use that data, hence making GDPR redundant and unnecessary in such cases (Comment PG: How is GDPR unnecessary just because you gather consent?).
	\closesection
	
	\hfill \\
	\noindent The idea of GDPR is to give users back the power over their own information and data and decide when it is used. Also because the use of data has become highly technical, obscure and unpredictable the concepts of consent which are the underlying base of the privacy rights are not meaningful as tracking the data in the world wide web and other services is very difficult (Comment PG: i don't see how consent is not meaningful, event though it might be complicated to controll...). Another approach was proposed in 2018 in the paper "A Blueprint for a Better Digital Society" by Jaron Lanier and E. Glen Weyl of "Data Dignity":
	...
	\chapter{Limitations}
	argumentation limitated by our own opinions, not necessarily clearly right or wrong.
	no investigation for other laws, that might interfere with gdpr
	...
